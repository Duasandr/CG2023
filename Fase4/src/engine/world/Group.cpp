    ...
    Group *Group::Create(tinyxml2::XMLElement *block) {
        ...
        if (models_block) {
            for (XMLElement *model_tag = models_block->FirstChildElement("model");
                ...
                std::string filePath = model_tag->Attribute("file");

                Model *model;
                if (model_tag->FirstChildElement("texture")) {
                    std::string texturePath = model_tag->FirstChildElement("texture")->Attribute("file");
                    model = Model::Create(filePath.c_str(), texturePath.c_str());
                } else {
                    model = Model::Create(filePath.c_str());
                }
        ...
        for (XMLElement *group_tag = block->FirstChildElement("group");
             group_tag != nullptr; group_tag = group_tag->NextSiblingElement("group")) {
